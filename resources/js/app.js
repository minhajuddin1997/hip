...
let js_error = data => Vue.swal("Error!",data,"error");
    ...
    var form_news = document.getElementById('signUpNewsLetter');
    
    console.log(form_news);
    
    form_news.addEventListener('submit', event => {
        event.preventDefault();
        document.getElementById('signNews').disabled = true;
        axios.post(APP_URL + '/newsLetter',new FormData(form_news))
            .then((response) => {
...
if (document.getElementById("signUp")) {
    var form_sign = document.getElementById('signUpForm');
    
    console.log(form_sign);
    
    form_sign.addEventListener('submit', event => {
        event.preventDefault();
        document.getElementById('signUpButton').disabled = true;
        axios.post(APP_URL + '/signup',new FormData(form_sign))
            .then((response) => {
                if (response.status === 200)
                {
                    window.jQuery('div#signUp button.close').trigger('click');
