...
//        $ip = 'IP: ' . $query['query'] . '<br>' . 'ZIP Code: ' . $query['zip'] . '<br>' . $query['country'] . '<br>' . $query['regionName'] . '<br>' . $query['city'] . '<br>' . 'Timezone: ' . $query['timezone'] . '<br>' . 'ISP: ' . $query['isp'] . '<br>' . 'Longitute: ' . $query['lon'] . '<br>' . 'Latitute: ' . $query['lat'] . '<br> Device: ' . $browser . '<br> User Date/Time: ' . $date;
        /*
         * Get Browser Data End
         * */
        $data_str = '1234567890abcefghijklmnopqrstuvwxyz';
        $autogenerated = substr(str_shuffle($data_str), 0, 7);
        $postRequest = array('name' => "{$request->input('first_name')} {$request->input('last_name')}",'email' => "{$request->input('email')}",'phone' => "{$request->input('phone')}",'client_company' => "{$request->input('company')}",'country' => $country,'city' => $city,'state' => $state,'client_password' => $autogenerated,'brand_id' => '2','client_division' => 'HireInPakistan');


        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://myprojectcrm.com/api/signup',
            ...
            CURLOPT_HTTPHEADER => array(
                'Cookie: PHPSESSID=078c2b3226b3793961b42c623c04e9ce'
            ),
        ));
...
//                        $email_params['ip'] = $ip;
            ...
            if (!empty($userId)) {
                ...
                $user->assignRole($request->input('role'));
                $user->save();
                $user_profile = Profile::select('id')->where('user_id', $userId)->get()->first();
...
//                        $email_params['ip'] = $ip;
    ...
    public function newsLetter(Request $request){
        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
            ...
            if (!empty($template->id)) {
                $template_data = EmailTemplate::getEmailTemplateByID($template->id);
                $email_params['name'] = 'Yet to register';
                $email_params['email'] = $request->email;
                $email_params['password'] = 'Yet to register';
//                        $email_params['ip'] = $ip;
                Mail::to($request->email)->send(new GeneralEmailMailable('new_user', $template_data, $email_params));
            }
        }
    }
...
//        $ip = 'IP: ' . $query['query'] . '<br>' . 'ZIP Code: ' . $query['zip'] . '<br>' . $query['country'] . '<br>' . $query['regionName'] . '<br>' . $query['city'] . '<br>' . 'Timezone: ' . $query['timezone'] . '<br>' . 'ISP: ' . $query['isp'] . '<br>' . 'Longitute: ' . $query['lon'] . '<br>' . 'Latitute: ' . $query['lat'] . '<br> Device: ' . $browser . '<br> User Date/Time: ' . $date;
        ...
        $data_str = '1234567890abcefghijklmnopqrstuvwxyz';
        $autogenerated = substr(str_shuffle($data_str), 0, 7);
        $postRequest = array('name' => "{$request->input('gs_first_name')} {$request->input('gs_last_name')}",'email' => "{$request->input('gs_email')}",'phone' => "{$request->input('gs_phone')}",'country' => $country,'city' => $city,'state' => $state,'client_password' => $autogenerated,'brand_id' => '2','client_division' => 'HireInPakistan');
        ...
        if ($response->level === "insert") {
            $user = new User;
            $user->first_name = $request->input('gs_first_name');
            $user->last_name = $request->input('gs_last_name');
            if (!empty($request->input('gs_email'))) {
                $user->email = filter_var($request->input('gs_email'), FILTER_SANITIZE_STRING);
            }
            $user->slug = filter_var($request->input('gs_first_name'), FILTER_SANITIZE_STRING) . '-' .
                filter_var($request->input('gs_last_name'), FILTER_SANITIZE_STRING);
            $user->password = \Hash::make($autogenerated);
            $user->user_verified = 1;
            $user->verification_code = null;
            $user->assignRole(3);
