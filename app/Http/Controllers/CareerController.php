...
class CareerController extends Controller
{
    ...
    public function index()
    {
        ...
        return View('career.index', compact('skills','locations'));
    ...
    public function signup(Request $request)
    {
        ...
        foreach($request->input('skills') as $skill ){
            $skill_id =  DB::table('skills')->where('title', $skill)->get();
            $skills[] = $skill_id[0]->id;
        } 
        ...
        if (!empty($request->file('banner_photo'))) {
            $banner_photo = $request->file('banner_photo');
            $image = rand().'.'.$banner_photo->getClientOriginalExtension();
            $banner_photo->move(public_path('uploads/career/'.$user->id),$image);
            $banner_photo = $image;
            $request->request->add(['banner' => $banner_photo]);
        }
        ...
        //For Skills
            ...
            for ($i=0; $i < count($skills); $i++){
                    DB::insert('insert into skill_user (skill_id, skill_rating, user_id) values (?, ?, ?)', [$skills[$i], $request->input('skills_percentage')[$i],$user->id]);
            }
